...
const HabitCell: React.FC<HabitCellProps> = ({ date, isActive, isCompleted, color, onToggle }) => {
  ...
  if (!isActive) {
    ...
    return (
      <div className="min-w-[3.5rem] h-14 sm:min-w-[2rem] sm:h-8 m-0.5 sm:m-1 rounded-xl bg-slate-100 dark:bg-slate-800 opacity-20 cursor-default flex-shrink-0 print:hidden" />
    );
  }
  ...
  return (
    ...
    >
      ...
      {isCompleted && (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 sm:h-5 sm:w-5 text-white drop-shadow-sm" viewBox="0 0 20 20" fill="currentColor">
