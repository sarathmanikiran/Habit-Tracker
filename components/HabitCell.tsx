...
const HabitCell: React.FC<HabitCellProps> = ({ date, isActive, isCompleted, color, onToggle }) => {
  ...
  if (!isActive) {
    ...
    return (
      <div className="min-w-[3rem] h-12 sm:min-w-[2rem] sm:h-8 m-0.5 sm:m-1 rounded-xl bg-slate-100 dark:bg-slate-800 opacity-20 cursor-default flex-shrink-0" />
    );
  }
  ...
  return (
    <button
      ...
      onClick={onToggle}
      className={`
        min-w-[3rem] h-12 sm:min-w-[2rem] sm:h-8 m-0.5 sm:m-1 rounded-xl transition-all duration-300 ease-out border flex-shrink-0
    ...
    >
      ...
      {isCompleted && (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 sm:h-5 sm:w-5 text-white drop-shadow-sm" viewBox="0 0 20 20" fill="currentColor">
